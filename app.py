...
# ===================  PAGE: MY RESULTS  ======================================
    ...
    if not bygroup:
        ...
        for i, g in enumerate(sorted(bygroup)):
            ...
            with gcols[i % 3]:
                if done_pairs:
                    # Build the ranking from whatever games are predicted so far.
                    # Partial tables are allowed — they just don't award bonus.
                    ...
                    if complete:
                        ...
                        note = (f"<small class='muted'>⚠️ Մասնակի աղյուսակ ({done}/{total} խաղ)։ "
                                f"Բոնուսի համար պետք է կանխատեսես բոլոր {total} խաղերը։</small>")
